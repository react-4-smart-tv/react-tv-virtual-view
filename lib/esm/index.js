...
var useVirtual = (function (_a) {
    ...
    var outerRectRef = useRef({ width: 0, height: 0 });
    ...
    var scrollToIndex = useCallback(function (val, cb, isSync) {
        var _a = isNumber(val) ? { index: val } : val, index = _a.index, _b = _a.align, align = _b === void 0 ? Align.auto : _b, smooth = _a.smooth;
        if (!isNumber(index))
            return;
    ...
    var scrollToItem = useCallback(function (val, cb) { return scrollToIndex(val, cb); }, [scrollToIndex]);
    var startItem = useCallback(function (idx, cb) { return scrollToIndex(idx, cb, true); }, [scrollToIndex]);
...
export { Align, useVirtual as default };
